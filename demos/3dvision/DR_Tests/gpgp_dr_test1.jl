...
function PROGRAM()
	...
	println(np.shape(V))
	# df_dtranslation = pyeval("npsum(f.dr_wrt(translation).A,0)/(w*h*1.0)",npsum=np.sum,f=f,translation=translation,w=WIDTH,h=HEIGHT)
	# println(df_dtranslation)

	# m=DiscreteUniform(2,6)
	# X=zeros(2*m)
	# for i=1:m
	# 	X[i]=Uniform(0,0.4)
	# end
	...
	# 	observe(Normal(X[k],0.0005),OBSERVATIONS[k])
	# end
	t2=time()
